    ...
    private function decorateTasks(Collection $tasks): Collection
    {
        ...
        return $tasks->map(function (Task $task) use ($articleStats, $runStats, $latestRuns, $titleNames, $modelNames): array {
            ...
            return [
                ...
                'article_type_options' => array_values(is_array($task->article_type_options ?? null) ? $task->article_type_options : ['explainer', 'comparison', 'decision', 'tutorial']),
                'writing_style_mode' => (string) ($task->writing_style_mode ?? 'random'),
                'writing_style_options' => array_values(is_array($task->writing_style_options ?? null) ? $task->writing_style_options : ['professional', 'consultant', 'editorial', 'educational', 'friendly']),
                'length_mode' => (string) ($task->length_mode ?? 'short'),
                'length_min' => $this->nullableInt($task->length_min),
                'length_max' => $this->nullableInt($task->length_max),
